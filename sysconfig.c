...
//#define OV7670_STREAM							0x43020000
#define VDMA_MM2S								0x43000000
#define VDMA_S2MM								0x43010000
#define FRAME_RANGE                             268435456
...
int vdma_setup(vdma_handle *handle, unsigned int baseAddr, int width, int height, int pixelLength, unsigned int fb1Addr) {
    ...
    //added according to tutorial example
    handle->vdmaHandler = open("/dev/mem", O_RDWR | O_SYNC);

    handle->vdmaVirtualAddress = (unsigned int *)mmap(0, getpagesize(), PROT_READ | PROT_WRITE, MAP_SHARED, handle->vdmaHandler, baseAddr);
    handle->fb1PhysicalAddress = (uint16_t *)fb1Addr;
    handle->fb1VirtualAddress = (uint16_t *) mmap(0, pixelLength*height*width, PROT_READ | PROT_WRITE, MAP_SHARED, handle->vdmaHandler, fb1Addr);

    memset(handle->fb1VirtualAddress, 0xFFFF, height*width*pixelLength); //set default memory values
    return 0;
}
...
int main() {
    ...
    // Initialize VDMA modules
    vdma_start_s2mm(&handle_s2mm);
    vdma_start_mm2s(&handle_mm2s);

    //expanded xil_out instruction
    int fd = open("/dev/mem", O_RDWR | O_SYNC);
    unsigned int *ovstream_reg = (unsigned int *)mmap(0, getpagesize(), PROT_READ | PROT_WRITE, MAP_SHARED, fd, OV7670_STREAM);
    *ovstream_reg = 1;
